...
type mLCache struct {
	...
	L1 *ristrettoStore.RistrettoStore
...
func ShakeTime10(t time.Duration) time.Duration {
	...
	// 计算时间抖动量
	jitter := int64(0.1 * float64(nano))
	// 生成介于 0 到抖动量之间的随机时间值
	randomJitter := jitter - rand.Int63n(2*jitter)
	// 添加时间抖动量到原始时间值上
	finalNanoseconds := nano + randomJitter
	// 将最终的时间值转换回 duration 类型
	finalDuration := time.Duration(finalNanoseconds)
